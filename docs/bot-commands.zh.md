...
输出内容包括：大小、内容类型、ETag、上传日期、Telegram file ID、缓存状态以及活跃的分享链接。
...
输出内容包括：总对象数、总大小、存储桶数量、活跃分享数、进行中的分段上传数。
