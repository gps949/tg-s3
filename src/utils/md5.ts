// Pure JS MD5 implementation for Cloudflare Workers (Web Crypto doesn't support MD5)
// Required for S3 ETag compatibility
...
function wordsToHex(words: number[]): string {
  ...
  for (const w of words) {
    for (let j = 0; j < 4; j++) {
      hex.push(((w >>> (j * 8)) & 0xff).toString(16).padStart(2, '0'));
    }
  }
  return hex.join('');
}
